...
class SendEmailPage(webapp.RequestHandler):
  ...
  def get(self):
    """
    HTTP GET handler.  Send an email to specified email address.  The email
    contains the salt that corresponds to the email.  If the email is not
    found in the database, just ignore. 
    """
    res = self.response
    ...
    if record:
      ...
      # the email.
      mail.send_mail('rogerta@gmail.com', 
                     record.email,
                     'Salt',
                     record.salt)
      status = 200
